...
export const getAppointmentInfo = async (appointmentId) => {
  ...
  const appointments = await fetch(`${baseURL}/appointment/${appointmentId}`, {
    cache: "no-store",
  }).then((res) => res.json());
  ...
  const patients = await fetch(`${baseURL}/patient/${patient}`, {
    cache: "no-store",
  }).then((res) => res.json());
  // traer especialista de la lista de especialistas
  const specialists = await fetch(`${baseURL}/specialist/${specialist}`, {
    cache: "no-store",
  }).then((res) => res.json());
